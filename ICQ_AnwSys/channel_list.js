           ...
           $.ajax({
				...
				success: function(data){
						$.each(data._embedded.channelList, function(i, item){
							...
							$('.channel_list').append('<li class="channel_item dropdown-item" id="'+item.id+'" onclick="update_users(this.id)"><strong>'+ item.name + '</strong>	#'+item.topic+'</li>');
	...
	function refreshPosts(x)
		{
		    ...
		    setTimeout(refreshPosts, x*1000);
		}
